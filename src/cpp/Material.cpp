...
bool CLinkMaterial::Read(ifstream& Input, unsigned int mset){
	Input >> nset;	// Number of property set;
  if (nset != mset + 1)
	{
		cerr << "*** Error *** Material sets must be inputted in order !" << endl
...
void CLinkMaterial::Write(COutputter& output, unsigned int mset){
	output << setw(5) << mset+1 << setw(16) << k<< endl;
}
