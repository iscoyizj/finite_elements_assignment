...
bool CBarMaterial::Read(ifstream& Input, unsigned int mset)
{
	...
	Input >> E >> Area >> rho;	// Young's modulus and section area and rho
...
//	Read material data from stream Input
bool CplaneMaterial::Read(ifstream& Input, unsigned int mset)
{
	Input >> nset;	// Number of property set
	...
	Input >> E >> mu;	// Young's modulus and poisson's rate
	Calculate_constitutive();
	return true;
}

//	Write material data to Stream
void CplaneMaterial::Write(COutputter& output, unsigned int mset)
{
	output << setw(5) << mset + 1 << setw(16) << E << setw(16) << mu << endl;
}

//  Calculate constitutive matrix
void CplaneMaterial::Calculate_constitutive() 
{
	Dmat = new double[4];
	Dmat[0] = E / (1 - mu * mu);
	Dmat[1] = Dmat[0];
