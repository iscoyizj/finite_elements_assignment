...
void CH8::WritePlotPost(COutPlotPost& output, unsigned int Ele)
{
	output << 8 << setw(11) << nodes_[0]->NodeNumber-1 << setw(9) << nodes_[1]->NodeNumber-1 << setw(9) << nodes_[3]->NodeNumber-1 
		   << setw(9) << nodes_[2]->NodeNumber-1 << setw(9) << nodes_[4]->NodeNumber-1 << 
		    setw(9) << nodes_[5]->NodeNumber-1 << setw(9) << nodes_[7]->NodeNumber-1 << 
...
void CH8::CalculateBe (double* Be, double psi, double eta, double zet)
{
...
void CH8::ElementStress(double* stress, double* Displacement)
{
	...
	CalculateBe(Be,Ga,-Ga,-Ga);
	CalculateStress(Be, stress+6, Displacement);
	
	CalculateBe(Be,Ga,Ga,-Ga);
	CalculateStress(Be, stress+12, Displacement);
	
	CalculateBe(Be,-Ga,Ga,-Ga);
	CalculateStress(Be, stress+18, Displacement);
	
	CalculateBe(Be,-Ga,-Ga,Ga);
	CalculateStress(Be, stress+24, Displacement);
	
	CalculateBe(Be,Ga,-Ga,Ga);
	CalculateStress(Be, stress+30, Displacement);
	
	CalculateBe(Be,Ga,Ga,Ga);
	CalculateStress(Be, stress+36, Displacement);
	
	CalculateBe(Be,-Ga,Ga,Ga);
	CalculateStress(Be, stress+42, Displacement);
}
...
void CH8::ElementCoord (double* coord)
{
	...
	CalculateCoord (coord+6, Ga, Ga, -Ga);
	CalculateCoord (coord+9, -Ga, Ga, -Ga);
	CalculateCoord (coord+12, -Ga, -Ga, Ga);
	CalculateCoord (coord+15, Ga, -Ga, Ga);
	CalculateCoord (coord+18, Ga, Ga, Ga);
	CalculateCoord (coord+21, -Ga, Ga, Ga);
	
}


void CH8::CalculateCoord (double* coord, double psi, double eta, double zet)
{
	double N[8];
...
void CH8::CalculateStress (double* Be, double* stress, double* Displacement)
{
...
void CH8::CalculateVolume (double* EG, double psi, double eta, double zet)
{
//GN matrix
    double GN[12];
...
void CH8::ElementPostInfo(double* stress, double* Displacement, double* PrePositions, double* PostPositions)
{
	for (unsigned int i=0;i<NEN_;i++)
	{
...
void CH8::CalculateMatrix (double* Be, double* Matrix)
{
	CH8Material* material_ = dynamic_cast<CH8Material*>(ElementMaterial_);
	double CM[3];
	CM[1]= material_->Lam;
